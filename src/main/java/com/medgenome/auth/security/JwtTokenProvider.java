...
class CachedSessionData {
    ...
    public CachedSessionData(String sessionId, List<String> permissions, String role, Integer tenantId) {
...
public class JwtTokenProvider {
    ...
    private boolean isAccessToken(Claims claims) {
        ...
        // If no explicit token_type, infer from claims presence
        // Access tokens should have permissions or role claims
    ...
    private CachedSessionData getCachedSessionDataBySessionId(String sessionId, String username) {
        ...
        if (cachedData != null && sessionId.equals(cachedData.getSessionId())) {
            return cachedData;
        }
    ...
    public TokenDetails parseToken(String token) {
        ...
        if (!isAccessToken && tenantId == null) {
            String sessionId = claims.get("sessionId", String.class);
            if (sessionId == null) {
                sessionId = claims.get("sid", String.class);
            }
            if (sessionId != null && username != null) {
                CachedSessionData cachedData = getCachedSessionDataBySessionId(sessionId, username);
                if (cachedData != null && cachedData.getTenantId() != null) {
                    tenantId = cachedData.getTenantId();
                    LOGGER.debug("Retrieved tenantId {} from cached session data for refresh token with sessionId {}", tenantId, sessionId);
                } else {
                    LOGGER.warn("Could not retrieve tenantId from cached session data for refresh token with sessionId {} and user {}", sessionId, username);
                }
            }
        }
        ...
        if (permissions == null || permissions.isEmpty()) {
            if (!isAccessToken) {
                ...
                if (validateSessionId && username != null) {
                    CachedSessionData cachedData = sessionDataCache.get(username);
                    if (cachedData != null && cachedData.getPermissions() != null && !cachedData.getPermissions().isEmpty()) {
                        permissions = cachedData.getPermissions();
                        LOGGER.debug("Using cached permissions for access token user {} from sessionId {}", username, cachedData.getSessionId());
                    } else {
        ...
        if (tenantObj != null) {
            ...
            // For refresh tokens, if tenantId was retrieved from cache, use it for tenant as well
            tenant = tenantId;
        }
    ...
    private boolean isTokenNotExpired(String token) {
        try {
            ...
            if (expObj instanceof Number) {
                expirationSeconds = ((Number) expObj).longValue();
            } else {
                expirationSeconds = Long.parseLong(expObj.toString());
            }
    ...
    private void validateSessionIdFromClaims(Claims claims) throws JwtException {
        ...
        // For access tokens, extract tenantId from claims
        ...
        if (isAccessToken) {
            ...
            if (tenantIdObj != null) {
                try {
                    ...
                    LOGGER.debug("Failed to parse tenant_id for caching: {}", tenantIdObj);
                }
            }
        } else {
            ...
            if (refreshTokenCachedData != null) {
                tenantId = refreshTokenCachedData.getTenantId();
                // Also get permissions and role from cache if not in token
                if ((permissions == null || permissions.isEmpty()) && refreshTokenCachedData.getPermissions() != null) {
                    permissions = refreshTokenCachedData.getPermissions();
                }
                if ((role == null || role.isEmpty()) && refreshTokenCachedData.getRole() != null) {
                    role = refreshTokenCachedData.getRole();
                }
        ...
        if (cachedData == null) {
            ...
            CachedSessionData newData = new CachedSessionData(sessionId, permissions, role, tenantId);
            ...
            if (!cachedData.getSessionId().equals(sessionId)) {
                LOGGER.warn("SessionId mismatch for user {}. Expected: {}, Found: {}", 
                        username, cachedData.getSessionId(), sessionId);
            ...
            List<String> permissionsToCache = (permissions != null && !permissions.isEmpty()) 
                    ? permissions 
                    : cachedData.getPermissions();
            String roleToCache = (role != null && !role.isEmpty()) ? role : cachedData.getRole();
            Integer tenantIdToCache = (tenantId != null) ? tenantId : cachedData.getTenantId();
            
            CachedSessionData updatedData = new CachedSessionData(sessionId, permissionsToCache, roleToCache, tenantIdToCache);
